...
from .relation_tool import PositionalEmbedding, RelationModule


@HEADS.register_module()
class TailBBoxHead(BBoxHead):
    ...
    def __init__(self,
                 ...
                 *args,
                 **kwargs):
        super(TailBBoxHead, self).__init__(*args, **kwargs)
        ...
        # self.nl_block = NLBlock(in_channels=256)
        ...
        # self.relations = nn.ModuleList([RelationModule(), RelationModule()])
    ...
    def init_weights(self):
        super(TailBBoxHead, self).init_weights()
    ...
    def forward(self, x):  # , rois=None):
        ...
        # if rois is not None:
        #     # print(rois.shape)
        #     position_embedding = PositionalEmbedding(rois[:, 1:])
        ...
        if self.num_shared_fcs > 0:
            ...
            for fc in self.shared_fcs:
                x = self.relu(fc(x))
                # if rois is not None:
                #     x = relation([x, position_embedding])
        # separate branches
        x_cls = x
        x_reg = x
